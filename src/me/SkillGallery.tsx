import { Card, Icon, SimpleGrid, Text } from "@chakra-ui/react";
...
const SkillGallery = () => {
  ...
  const skills = [
    ...
    {
      name: "JSX",
      icon: <Text fontFamily="mono" fontSize="3xl" fontWeight="extrabold" letterSpacing="tighter">JSX</Text>,
    },
  ];
    ...
    <SimpleGrid columns={{ base: 2, md: 4 }} gap="4">
      {skills.map((skill) => (
        <Card.Root
          ...
          cursor="default"
          key={skill.name}
          _hover={{ shadow: "lg", scale: 1.15 }}
          transition="all 0.2s ease-out"
          textAlign="center"
        >
          <Card.Body gap="4" alignItems="center" justifyContent="space-around">
            {skill.icon}
            <Card.Description>{skill.name}</Card.Description>
          </Card.Body>
        </Card.Root>
      ))}
    </SimpleGrid>
